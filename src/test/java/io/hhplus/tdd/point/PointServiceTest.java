...
class PointServiceTest {
    ...
    void charge_by_id_return_charged_point_1100() {
        ...
        // when
        UserPoint userPoint = pointService.charge(id, amount * 10);
        // then
    ...
    void use_by_over_point_throw_exception() {
        ...
        pointService.charge(id, amount);
        // when, then
        assertThrows(IllegalArgumentException.class, () -> pointService.use(id, amount * 2));
    }
    ...
    void use_by_id_return_remain_point() {
        ...
        pointService.charge(id, amount);
        // when
        UserPoint userPoint = pointService.use(id, amount / 2);
        // then
    ...
    void history_by_id_return_empty_list() {
        ...
        long id = 1L;
        // when
        var histories = pointService.history(id);
        // then
    ...
    void history_by_negative_id_throw_exception() {
        ...
        long id = -1L;
        // when, then
        assertThrows(IllegalArgumentException.class, () -> pointService.history(id));
    }
    ...
    void charge_concurrently() throws InterruptedException {
        ...
        new Thread(() -> { // 새로운 스레드에서
            pointService.charge(id, amount); // 포인트를 충전하고,
        ...
        new Thread(() -> { // 또 다른 새로운 스레드에서
            try {
                ...
                pointService.use(id, amount); // 그리고 포인트를 사용합니다.
        ...
        assertEquals(0, pointService.point(id).point()); // 그리고 사용자의 포인트가 0인지 확인합니다.
    }
}